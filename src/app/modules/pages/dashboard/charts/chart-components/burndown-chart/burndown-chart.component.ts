...
export class BurndownChartComponent implements OnInit {
  ...
  initializeChart() {
    ...
    // Count remaining values for each day
    ...
    days.forEach(day => {
      ...
      const itemsToCount = sprintItems?.filter(i => !i.done_date || moment(i.done_date).isAfter(day, 'day'));
      let dayValue = 0;
      if (itemsToCount) {
    ...
    for(let i = 0, j = 0; i <= numberOfDaysToParse; i++) {
      ...
      if (this.showWeekends && (day.isoWeekday() === 6 || day.isoWeekday() === 7)) {
        ...
        chartIdeal.push(chartData[0] - (chartData[0] / (numberOfDays - numberOfWeekendDays)) * j);
        j++;
      }
    }
